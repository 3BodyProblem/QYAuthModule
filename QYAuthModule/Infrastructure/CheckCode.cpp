...
unsigned short	CRC16::scrctable[256] = {	0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7,
...
unsigned short CRC16::CheckCode(const char * lpInBuf,unsigned long sInSize, unsigned short wLastCk)
{
...
unsigned long CRC32::lcrctable[256] = {0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA,
...
unsigned long CRC32::CheckCode(const char * lpInBuf,unsigned long sInSize, unsigned long dwLastCk)
{
...
ConCheckCode::ConCheckCode(void)
{
	
}

ConCheckCode::~ConCheckCode()
{
	
}

unsigned short ConCheckCode::CheckCode(char * lpInBuf,unsigned long sInSize)
{
...
DWORD  MD5::inner_f(DWORD X,DWORD Y,DWORD Z)
{
    return((X & Y) | ((~X) & Z));
}

DWORD  MD5::inner_g(DWORD X,DWORD Y,DWORD Z)
{
    return((X & Z) | (Y & (~Z)));
}

DWORD  MD5::inner_h(DWORD X,DWORD Y,DWORD Z)
{
    return(X ^ Y ^ Z);
}

DWORD  MD5::inner_i(DWORD X,DWORD Y,DWORD Z)
{
    return(Y ^ (X | (~Z)));
}

DWORD  MD5::inner_shrloop(DWORD Value,unsigned char N)
{
    return((Value << N) | (Value >> (32-N)));
}

void  MD5::inner_ff(DWORD &A,DWORD B,DWORD C,DWORD D,DWORD X,unsigned char S,DWORD T)
{
...
void  MD5::inner_gg(DWORD &A,DWORD B,DWORD C,DWORD D,DWORD X,unsigned char S,DWORD T)
{
...
void  MD5::inner_hh(DWORD &A,DWORD B,DWORD C,DWORD D,DWORD X,unsigned char S,DWORD T)
{
...
void  MD5::inner_ii(DWORD &A,DWORD B,DWORD C,DWORD D,DWORD X,unsigned char S,DWORD T)
{
...
int  MD5::CalMD5(const char * lpInBuf,int iInSize,char szOutBuf[16])
{
    DWORD                   aa,bb,cc,dd;
	char szMd5Key[16] = {0};
    ...
    return CalMD5(szMd5Key, lpInBuf, iInSize, szOutBuf);
}

int MD5::CalMD5(const char in_OldMd5[16], const char * lpInBuf,int iInSize,char szOutBuf[16])
{
